...
class HybridTokenMixer(nn.Module):  ### D-Mixer
    def __init__(self,
        ...
        if use_mca:
            ...
            mca_dim = dim // 2
            if mca_dim % 4 == 0:
                scales = [1, 2, 4]  # 适用于可被4整除的维度
            elif mca_dim % 3 == 0:
                scales = [1, 2, 3]  # 适用于可被3整除的维度
            elif mca_dim % 2 == 0:
                scales = [1, 2]  # 适用于可被2整除的维度
            else:
                scales = [1]  # 默认单尺度
...
class Block(nn.Module):
    ...
    Args:
        ...
        layer_scale_init_value (float): Init value for Layer Scale.
            Defaults to 1e-5.
        use_mca (bool): Whether to use Multi-Scale Coordinate Attention. Defaults to True.
    """
...
def basic_blocks(dim,
    ...
    for block_idx in range(layers[index]):
        ...
        # 只在每个stage的前半部分块中使用MCA，平衡性能和计算成本
        ...
        blocks.append(
            Block(
                ...
                use_mca=block_use_mca,  # 传递MCA使用标志
            ))
    return blocks
...
class TransXNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(layers)):
            # 深层stage减少MCA使用以控制计算复杂度
            stage_use_mca = use_mca and (i < 3)  # 只在stage0-2使用MCA
...
def transxnet_t(pretrained=False, pretrained_cfg=None, **kwargs):
    """TransXNet-T with Multi-Scale Coordinate Attention"""
    model = TransXNet(arch='t', use_mca=True, **kwargs)
...
def transxnet_s(pretrained=False, pretrained_cfg=None, **kwargs):
    """TransXNet-S with Multi-Scale Coordinate Attention"""
    model = TransXNet(arch='s', use_mca=True, **kwargs)
    model.default_cfg = _cfg(crop_pct=0.9)
    return model
...
def transxnet_b(pretrained=False, pretrained_cfg=None, **kwargs):
    """TransXNet-B with Multi-Scale Coordinate Attention"""
    model = TransXNet(arch='b', use_mca=True, **kwargs)
